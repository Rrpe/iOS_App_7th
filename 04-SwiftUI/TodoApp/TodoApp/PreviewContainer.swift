...
class PreviewContainer {
    ...
    func insertPreviewData() {
        ...
        let todos: [(String, Date, Priority, Date, Category?)] = [
            ...
            ("Do the laundry", calendar.date(byAdding: .day, value: 2, to: today)!, .medium, dueDate, categories[1]),
            ("Take out the trash", calendar.date(byAdding: .day, value: 3, to: today)!, .low, dueDate, categories[1]),
            ("완료된 작업", calendar.date(byAdding: .day, value: 4, to: today)!, .low, dueDate, categories[2]),
