...
class UserNameHandler: HTTPServerDelegate {
    // 유지보수 카운터 (클래스 프로퍼티)
    static var maintenanceCounter = 0
        ...
        if request.path == "/isUserNameAvailable" {
            ...
            if let userName = request.queryParams["userName"] {
                ...
                if userName == "maintenance" {
                    UserNameHandler.maintenanceCounter += 1
                    print("Maintenance counter: \(UserNameHandler.maintenanceCounter)")
                    
                    if UserNameHandler.maintenanceCounter % 3 != 0 {
                ...
                if userName.isEmpty {
                    return HTTPResponse(
                        ...
                        body: "Bad Request: userName 이 비어있습니다."
                    )
                }
                ...
                if userName.count < 3 {
                    return HTTPResponse(
                        statusCode: 400,
                        headers: ["Content-Type": "text/plain"],
                        body: "Bad Request: userName은 최소 3자 이상이어야 합니다."
                    )
                }
